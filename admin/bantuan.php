        ...
        <div class="col-xs-12">
          <div class="box">
            ...
            if(isset($_GET["edit"])){
              ...
              $id_konten = $pecah[1];
              $data_konten = $konek->prepare("SELECT * FROM bantuan WHERE id_bantuan=?");
              ...
              ?>
                <div class="box-header">
                  <h3 class="box-title">Edit Bantuan
                    <a href="bantuan" class="btn btn-primary">Kembali</a>
                  </h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                  <form role="form" action="?update=<?= $page; ?>" method="post">
                    <div class="form-group">
                      ...
                      <input type="text" value="<?= $tampil_data["nama_bantuan"]; ?>" name="judul" class="form-control" id="Judul File" placeholder="Masukan NAma Status">
                    </div>
              ...
              $update = $konek->prepare("UPDATE bantuan SET nama_bantuan=? WHERE id_bantuan=?");
            ...
            ?>
              <div class="box-header">
                <h3 class="box-title">Daftar Bidang Minat
              ...
              <div class="box-body">
                ...
                <table id="datafile" class="table table-bordered table-striped">
                  ...
                  <tr>
                    <th width="10%">No</th>
                    <th width="60%">Nama Status</th>
                    <th width="30%">Opsi</th>
                  </tr>
                  </thead>
                  <tbody>
                  <?php
                  $data_konten = $konek->prepare("SELECT * FROM bantuan ORDER BY nama_bantuan DESC");
                  $data_konten->execute();
                  $tampung_konten = $data_konten->get_result();
                  $nourut = 1;
                  while($tampil_data = $tampung_konten->fetch_assoc()){
                    $buatedit = "data#$tampil_data[id_bantuan]#Azzam#0813";
                    $enedit = tutupkunci($buatedit);
                  ?>
                  <tr>
                    <td><?= $nourut; ?></td>
                    <td><?= $tampil_data["nama_bantuan"]; ?></td>
                    <td>
